...
export async function adjudicateClaim(
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === 'AbortError') {
      throw new Error('Request was cancelled or timed out.', { cause: err });
    }
    throw err;
  }
}
...
export async function fetchClaimsHistoryFromDb(policyId: string, memberId: string): Promise<DbClaimsHistory> {
  ...
  return response.json() as Promise<DbClaimsHistory>;
}

/**
 * Fetch benefit balances details from the backend's external ledger endpoint.
 */
export async function fetchBalancesFromDb(policyId: string, memberId: string): Promise<DbBenefitBalance> {
  ...
  return response.json() as Promise<DbBenefitBalance>;
}

/**
 * Fetch lifetime state details from the backend's external ledger endpoint.
 */
export async function fetchLifetimeStateFromDb(policyId: string): Promise<DbLifetimeState> {
  ...
  return response.json() as Promise<DbLifetimeState>;
}

/**
 * Fetch endorsements details from the backend's external ledger endpoint.
 */
export async function fetchEndorsementsFromDb(policyId: string): Promise<{ endorsements: EndorsementData[] }> {
  ...
  return response.json() as Promise<{ endorsements: EndorsementData[] }>;
}

/**
 * Fetch porting details from the backend's external ledger endpoint.
 */
export async function fetchPortingFromDb(policyId: string): Promise<DbPortingMigration> {
  ...
  return response.json() as Promise<DbPortingMigration>;
}
